...
A：我刚刚找了下，好像是这样。因为内核和我们平时写的用户态程序不同，内核一般是用特定的寄存器来存参数（a0-a7好像是，8个寄存器）来让用户态的参数传到内核态，然后内核态再通过`arg`（如我们刚刚用的`argint`）什么的从寄存器提取出来实际的参数。

Q：`usys.pl`文件是什么呀？是汇编吗？不过看着也不像呀。    
A：首先，他不是汇编文件，是`perl`脚本。他的作用...类似一个中间人，他会生成`usys.S`，他会做两件事：
  - 准备参数：他会把你要调用的那个系统调用号放到`a7`寄存器。
  - 去找内核：他会带着系统调用号去找内核，然后把系统调用号送给内核。
